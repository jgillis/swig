...
protected:
  ...
  /* Driven by %feature("abstract"); C++ abstract bases still have storage. */
...
int MATLAB::top(Node *n) {
  ...
  Printf(f_runtime, "\n");
  Swig_obligatory_macros(f_runtime, "MATLAB");
  ...
  Printf(f_begin,
         "    SWIG_Error_Format(SWIG_RuntimeError, \"This mex file should only be called from "
...
int MATLAB::functionWrapper(Node *n) {
  ...
  Printf(f->code,
         "if (!SWIG_check_num_args(\"%s\",argc,%i,%i,%i)) "
         "{\n SWIG_fail;\n }\n",
         iname,
         num_arguments,
         num_required,
  ...
  for (j = 0, p = l; j < num_arguments; ++j) {
    ...
    if (tm) {
      ...
      if (Getattr(p, "wrap:disown") || (Getattr(p, "tmap:in:disown"))) {
        Replaceall(tm, "$disown", destructor ? "SWIG_MATLAB_POINTER_DELETE" : "SWIG_POINTER_DISOWN");
      } else {
  ...
  Wrapper_add_local(f, "_out", "mxArray * _out = 0");
  ...
  emit_isvoid_special_variables(n, f->code, SwigType_type(d) == T_VOID);
...
int MATLAB::classDirectorMethod(Node *n, Node *parent, String *super) {
  ...
  if (ignored_method) {
    ...
    /* Convert callback arguments to MATLAB arrays. */
    ...
    if (!is_void) {
      ...
      if (tm != 0) {
        if (outputs > 1) {
          Printf(w->code, "output = dispatch_out[%d];\n", idx++);
...
int MATLAB::classHandler(Node *n) {
  ...
  /* Explicit abstract mixins contain MATLAB methods but no native pointer storage. */
  is_abstract_class = GetFlag(n, "feature:abstract");
  ...
  if (baselist) {
    ...
    for (Iterator b = First(baselist); b.item; b = Next(b)) {
      ...
      String *bpkg = bpkgNode ? Getattr(bpkgNode, "name") : 0;
      if (!bname || !bpkg || GetFlag(b.item, "feature:ignore"))
        continue;
      base_count++;

      bool base_is_abstract = GetFlag(b.item, "feature:abstract");
      if (!base_is_abstract)
...
void MATLAB::initConstant() {
  ...
  Printf(f_constants,
         "    SWIG_Error_Format(SWIG_RuntimeError, \"This function should only be called from inside the .m files generated by SWIG. First input should be the "
         "constant ID .\");\n");
...
void MATLAB::wrapConstructorDirector(int gw_ind, String *symname, String *fullname, Node *n) {
  ...
  if (fullname == 0) {
    ...
    if (have_matlabprepend(n))
      Printf(f_wrap_m, "%s\n", matlabprepend(n));
    Printf(f_wrap_m, "        if strcmp(class(self),'%s.%s')\n", pkg_name, symname);
...
int MATLAB::staticmemberfunctionHandler(Node *n) {
  ...
  if (Equal(Getattr(n, "feature:callback"), "%s")) {
    Printf(static_methods, "    function value = %s()\n", Getattr(n, "sym:name"));
    Printf(static_methods, "      value = %s(0, %d);\n", mex_name, con_id);
...
int MATLAB::insertDirective(Node *n) {
  ...
  if (Cmp(section, "matlab") == 0 && f_wrap_m) {
    ...
    if (!f_setup_m) {
      String *filename = NewStringf("%s%ssetup.m", SWIG_output_directory(), module_name);
      f_setup_m = NewFile(filename, "w", SWIG_output_files());
      if (!f_setup_m) {
...
void MATLAB::checkValidSymName(Node *node) {
  ...
  if (symname && !Strncmp(symname, "_", 1)) {
    Swig_warning(WARN_LANG_IDENTIFIER, Getfile(node), Getline(node), "MATLAB identifiers cannot start with '_'. Use %%rename to rename '%s'.\n", symname);
  }
}
