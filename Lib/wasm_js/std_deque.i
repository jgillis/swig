...
namespace std {
  template <class T, class Allocator = std::allocator<T> > class deque {
    %wasm_container(std::deque<T, Allocator>)
  public:
    typedef T value_type;
    deque();
    deque(unsigned long count);
    deque(unsigned long count, T value);
    ...
    %extend {
      ...
      void insert(unsigned long index, T value) {
        ...
        typename std::deque<T, Allocator>::iterator it = $self->begin();
      ...
      T at(unsigned long index) const {
        if (index >= $self->size()) throw std::out_of_range("sequence index out of range");
        typename std::deque<T, Allocator>::const_iterator it = $self->begin();
        std::advance(it, index);
        return *it;
      }
